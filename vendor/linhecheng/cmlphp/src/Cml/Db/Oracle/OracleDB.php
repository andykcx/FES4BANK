<?php
/* * *********************************************************
 * @Author  kechangxu<157262140@qq.com>
 * @Date: 18-8-22 
 * cmlphp框架 Oralce数据库 Pdo驱动类
 * *********************************************************** */

namespace Cml\Db\Oracle;
 ...
 * Orm Oracle数据库Pdo实现类
 *
 * @package Cml\Db\Oracle
 */
class OracleDB extends Base
{
    /**
     * 启用数据缓存
     *
    ...
    {
        $stmt = $this->prepare('SELECT * FROM ALL_USERS;', $this->rlink);
        $this->execute($stmt);
    ...
    public function get($key, $and = true, $useMaster = false, $tablePrefix = null)
    {
        ...
        $sql = "SELECT * FROM {$tableName} WHERE {$condition} AND ROWNUM <= 1000";
     ...
     * @param null $rownum 返回的条数
     * @param bool $isSelect 是否为select调用， 是则不重置查询参数并返回cacheKey/否则直接返回sql并重置查询参数
     *
     * @return string|array
     */
    public function buildSql($offset = null, $rownum = null, $isSelect = false)
    {
        is_null($offset) || $this->limit($offset, $rownum);
        ...
        empty($this->sql['limit']) && ($this->sql['limit'] = "WHERE ROWNUM <= 100");
     ...
     * @param int $rownum 返回的条数
     * @param bool $useMaster 是否使用主库 默认读取从库
     *
     * @return array
     */
    public function select($offset = null, $rownum = null, $useMaster = false)
    {
        list($sql, $cacheKey) = $this->buildSql($offset, $rownum, true);
    ...
    public function connect($host, $username, $password, $dbName, $charset = 'utf8', $engine = '', $pConnect = false)
    {
        ...
        try {
			...
			$tns = "  
				(DESCRIPTION =
					(ADDRESS_LIST =
					  (ADDRESS = (PROTOCOL = TCP)(HOST = ".$host[0].")(PORT = " . (isset($host[1]) ? "{$host[1]}" : '1521') . "))
					)
					(CONNECT_DATA =
					  (SERVICE_NAME = ".$dbName.")
					)
