...
console.log('in active reload.');	 
      ...
      table.reload('testReload', {
		...
		,url:'<?php \Cml\Http\Response::url("bank/Search/applySearch");?>?t='+ new Date()
		,parseData: function(res){ //res 即为原始返回的数据
				console.log(res);
				return {
				  "code": res.data.code, //解析接口状态
  ...
  table.on('tool(test)', function(obj){
    ...
    if(obj.event === 'del'){
		...
		layer.confirm('确定要删除该记录吗?', {
			  btn: ['是的','放弃'] //按钮
				...
				$.ajax({
					type:'POST',
					dataType:'json',
					url:'<?php \Cml\Http\Response::url("bank/Search/delApplyRecord");?>',
					data:{slbh:slbh},
					async : false,
		...
		var index = layer.open({
					...
					maxmin: true,
					area: ['800px', '500px'],
					content: '<?php \Cml\Http\Response::url("bank/Search/showDetail?slbh=");?>' + slbh,
					end: function(){
					}
				 });
		layer.full(index);
		...
		layer.open({
					...
					maxmin: true,
					area: ['800px', '500px'],
					content: '<?php \Cml\Http\Response::url("bank/Search/editApply");?>',
					end: function(){
					}
				 });
		...
		layer.confirm('确认提交申请?', {
			  btn: ['是的','放弃'] //按钮
				...
				$.ajax({
					...
					dataType:'json',
					async : false,
					url:'<?php \Cml\Http\Response::url("bank/Search/sendApplyRecord");?>',
					data:{'slbh':slbh},
					success:function(result){
