...
namespace bank\Controller\Acl;

use bank\Model\Acl\AccessModel;
use bank\Service\AclService;
use bank\Service\Builder\FormBuildService;
use bank\Service\Builder\GridBuildService;
...
use bank\Model\Acl\GroupsModel;
...
class UsersController extends CommonController
{
    ...
    {
        GridBuildService::create('web/Acl/Users/ajaxPage', '用户列表')
            ->addSearchItem('username', '请输入用户名')
            ->addSearchItem('nickname', '请输入用户昵称')
            ->addTopButton('新增用户', FormBuildService::BUTTON_ADD, 'web/Acl/Users/add', '新增用户', 'web/Acl/Users/save')
            ->addColumn('id', 'id')
    ...
    public function editSelfInfo()
    {
        ...
        $inst = FormBuildService::create('个人资料')
            ->addFormItem('姓 名', 'nickname', '请输入姓名', '',
                'lay-verify="length" lay-min="3" lay-max="20" ' . isset($showField['nickname']) ? '' : ' disabled '
            )
			->addFormItem('所属机构', 'company', '', '',
                'lay-verify="length" lay-min="3" lay-max="20"  disabled '
            )
			->addFormItem('登录用户名', 'username', '', '',
                'lay-verify="length" lay-min="3" lay-max="20"  disabled '
            );
        if (!$showField || $showField['password']) {
