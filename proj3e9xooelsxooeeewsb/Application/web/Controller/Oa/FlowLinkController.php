...
class FlowLinkController extends Controller
{
    ...
    public function dept(){
		...
		$dept = $deptModel->getDeptNameList();
		$depts = $deptModel->recursion($dept);
		
		$view = View::getEngine('Html');
		$view->assign('depts', $depts);
        $view->display('Oa/flowDesign/flowlink/dept');
    ...
    public function emp(){
		...
		$process = $processModel->getProcessInfo($processId);
		$deptModel = new DeptModel();
		$dept = $deptModel->getDeptNameList();
		$depts = $deptModel->recursion($dept);
		$empModel = new EmpModel();
		$emps = $empModel->getUsersList();
		$flowlinkModel = new FlowLinkModel();
		$auditors = $flowlinkModel->getAuditors($processId);
		$select_emps = $empModel->getEmpsInfo($auditors);
//\Cml\dump($emps);		
		$view = View::getEngine('Html');
		$view->assign('depts', $depts);
		$view->assign('emps', $emps);
		$view->assign('select_emps', $select_emps);
        $view->display('Oa/flowDesign/flowlink/emp');
